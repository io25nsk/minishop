...
async def test_add_cart(client):
    response = await client.post(f'/cart/add/',
                                 json={"uid": exist_uid,
                                       "pid": exist_pid,
                                       "quantity": 3})
...
async def test_create_order(client):
    global new_oid
    response = await client.post(f'/order/create/',
                                 json={'uid': exist_uid,
...
async def test_pay_order(client):
    response = await client.post(f'/order/pay/',
                                 json={'oid': new_oid,
                                       'pay_system': 'VISA'})
    assert 'error' not in response.json()
...
async def test_order_return_expired(client):
    response = await client.post(f'/order/{expired_oid}/return/', json={"pid": exist_pid, 'quantity': 1})
    assert 'error' in response.json()


@pytest.mark.anyio
async def test_order_return_more_quantity(client):
    response = await client.post(f'/order/{new_oid}/return/', json={"pid": exist_pid, 'quantity': 100})
    assert 'error' in response.json()


@pytest.mark.anyio
async def test_order_return(client):
    response = await client.post(f'/order/{new_oid}/return/', json={"pid": exist_pid, 'quantity': 1})
    assert 'error' not in response.json()


@pytest.mark.anyio
async def test_order_return_not_product(client):
    response = await client.post(f'/order/{exist_oid}/return/', json={"pid": not_return_pid, 'quantity': 1})
    assert 'error' in response.json()
...
#     response = client.get('/order/671210a24c0b7d4a8caa715a/')
